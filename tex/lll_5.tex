...
\begin{figure}[!htp]
	\centering
	\includegraphics[width=10cm]{example/zhuijijiao.jpg}
...
在一对一实验中，给红蓝两方一个初始位置分别为$(2,3)$和$(7,7)$,红方初速度为向上，蓝方初速度为向下。这里人控制红方，蓝方为学习到的智能体模型。红蓝两方的初始状态如图\ref{fig1:yiduiyi1-2:a}所示。红方为先行玩家，首先让红方向右走一步，得到蓝方的动作概率，如图\ref{fig1:yiduiyi1-2:b}。
...
\begin{figure}[!htbp]
	\centering
	\includegraphics[width=\hsize]{example/feijichushiweizhi.jpg}
	\bicaption[这里将出现在插图索引]
	{飞机初始位置示意图}
...
%\begin{figure}[!hbtp]
%	\centering
%	\includegraphics[width=10cm]{example/feijiyiduiyi1.jpg}
%	\bicaption[这里将出现在插图索引]
%	{红蓝两方初始状态}
%	{Neural network training pipeline}
%	\label{yiduiyi1.jpg}
...
%\begin{figure}[!hbtp]
%	\centering
%	\includegraphics[width=10cm]{example/feijiyiduiyi2.jpg}
%	\bicaption[这里将出现在插图索引]
%	{红蓝两方初始状态}
%	{Neural network training pipeline}
%	\label{yiduiyi2.jpg}
...
\begin{figure}[!htp]
	\centering
	\includegraphics[width=10cm]{example/feijiduoduiduo2.jpg}
	\bicaption[这里将出现在插图索引]
	{蓝方起始状态信息}
	{Neural network training pipeline}
